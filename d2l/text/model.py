...
from d2l.text.block import RNNsBlock, LSTMsBlock
...
class LanguageModel(nn.Module, ABC):
    ...
    def one_hot(self, input: torch.Tensor) -> torch.Tensor:
        """
        将词元索引转换为独热编码。
        返回形状 (time_steps, batch_size, vocab_size) 的张量。
        """
        return torch.nn.functional.one_hot(input.T, num_classes=self.vocab_size).float()

    def loss(self,
             output_logits: torch.Tensor,
             target: torch.Tensor) -> torch.Tensor:
        """计算交叉熵损失"""
        logits_reshaped = output_logits.reshape(-1, self.vocab_size)
        target_reshaped = target.reshape(-1)
        return torch.nn.functional.cross_entropy(logits_reshaped, target_reshaped)
    
    def predict(self, 
                input: torch.Tensor,
                states: Optional[Any] = None) -> Tuple[torch.Tensor, Any]:
        """预测下一个词元"""
        self.eval()
        with torch.no_grad():
            output_logits, final_states = self.forward(input, states)
